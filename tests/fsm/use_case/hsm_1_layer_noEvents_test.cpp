...
/* Initial state */
state_t onState = {.itsInitialState = NULL,
                   .itsParentState  = NULL,
                   .onEntry         = NULL,
                   .during          = NULL,
                   .onExit          = NULL,
                   .itsTransition =
                       (hsm_transition_t[]){{NULL, NULL, &offState}},
                   .itsTransitionNum = 1};

/* Next state */
state_t offState = {.itsInitialState = NULL,
                    .itsParentState  = NULL,
                    .onEntry         = NULL,
                    .during          = NULL,
                    .onExit          = NULL,
                    .itsTransition =
                        (hsm_transition_t[]){{NULL, NULL, &onState}},
                    .itsTransitionNum = 1};

static state_t* stateList[] = {&onState, &offState};
...
static void CHECK_HSM_NO_EVENT(hsm_t*        me,
                               state_t*      fromState,
...
TEST(hsm_1_layer_simple, Should_HandleSimpleOnOffHSMWithNoEvents_old)
{
	...
	/* ON.during -- > ON.checking_quard */
	{
		const int err = hsm_handleEvent(&me, NULL);
		POINTERS_EQUAL(&onState, me.itsCurrentState);
		LONGS_EQUAL(HSM_ST_M_CHECKING_GUARD,
		            me.itsCurrentState->itsMode);
		CHECK_EQUAL(1, err);
	}
