...
typedef struct
{
	bool const (*const guard)( const state_t* me, const hsm_event_t* event );		/**< The transitions's guard. */
	void const (*const action)( const state_t* me, const hsm_event_t* event );	/**< The transitions's action. */
...
struct state
{
	...
	const state_t* itsInitialState;												/**< If it has children this is the initial. */
	const state_t* const itsParentState;										/**< The state's parent. */
	int const (*const onEntry)( const state_t* me, const hsm_event_t* event );	/**< The state's on entry action. */
	int const (*const during)( const state_t* me, const hsm_event_t* event );	/**< The state's during action. */
	int const (*const onExit)( const state_t* me, const hsm_event_t* event );	/**< The state's on exit action. */
	const hsm_transition_t* const itsTransition;								/**< The state's transition. */
 ...
 * Basic
 */
hsm_t	_hsm_build( const state_t* initialState, state_t* allStates[], const uint32_t allStatesSize );
